...
CREATE OR REPLACE FORCE VIEW "SIAT_CT"."MINADER_FILES" (
    ...
    WHERE
        f."BUREAU_ID" IS NOT NULL AND f.file_type_id IN (
...
CREATE OR REPLACE FORCE VIEW MINADER_FILES_TRACKING
AS SELECT F.NUMERO_DEMANDE, F.NUMERO_DOSSIER, FT.CODE FILE_TYPE_CODE, FT.LABEL_FR FILE_TYPE_NAME_FR, FT.LABEL_EN FILE_TYPE_NAME_EN, F.CREATED_DATE, C.NUM_CONTRIBUABLE, C.COMPANY_NAME, TPC.VALUE "TYPE_PRODUIT_CODE", TPN.VALUE "TYPE_PRODUIT_NOM",
E.ID BUREAU_ID, E.CODE BUREAU_CODE, AD.LABEL_FR BUREAU_NAME_FR, AD.LABEL_EN BUREAU_NAME_EN FROM FILES F
...
JOIN FILE_FIELD TPCFF ON TPCFF.FILE_TYPE_ID = FT.ID AND TPCFF.CODE = 'TYPE_PRODUIT_CODE'
JOIN FILE_FIELD_VALUE TPC ON TPC.FILE_FIELD_ID = TPCFF.ID
JOIN FILE_FIELD TPNFF ON TPNFF.FILE_TYPE_ID = FT.ID AND TPNFF.CODE = 'TYPE_PRODUIT_NOM'
...
WHERE F.BUREAU_ID IS NOT NULL AND F.NUMERO_DEMANDE IS NOT NULL;
