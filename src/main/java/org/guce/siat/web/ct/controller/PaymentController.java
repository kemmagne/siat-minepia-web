...
public class PaymentController extends AbstractController<FileItem> {
    ...
    public static final String NATURE_FRAIS_CP = "Frais consentement pr̩alable";
    ...
    public void goToCostsPage() {
        try {
            ...
            }  else if (isVtMinepded(selectedFile.getFile())) {
                paymentData = getPaymentDataByFileAndPaymentFlow(selectedFile.getFile(), FlowCode.FL_AP_VT2_01);
            } else {
    ...
    public List<FileDto> getFileDtoList() {
        ...
        for (final File paymentFile : paymentFileList) {
            ...
            }else if (isAtmMinepia(paymentFile)) {
                paymentData = getPaymentDataByFileAndPaymentFlow(paymentFile, FlowCode.FL_AP_ATM_01);
            }  
            
            else if (isVtMinepded(paymentFile)) {
                paymentData = getPaymentDataByFileAndPaymentFlow(paymentFile, FlowCode.FL_AP_VT2_01);
            }else {
    ...
    public boolean isVtMinepded(File currentFile) {
        return FileTypeCode.VT_MINEPDED.equals(currentFile.getFileType().getCode());
    }
    
    public boolean isAtmMinepia(File currentFile) {
        return FileTypeCode.ATM_MINEPIA.equals(currentFile.getFileType().getCode());
    }
