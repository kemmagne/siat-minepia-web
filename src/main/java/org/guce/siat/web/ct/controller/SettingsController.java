...
public class SettingsController extends AbstractController<ParamsOrganism> {
     ...
     */
    private static final String BOOLEAN_ERROR = "BooleanError";
    ...
    public synchronized void edit() {
        if (ParamsNames.MAX_ATTEMPTS_USER_CONNEXION.getCode().equals(selectedParam.getName())) {
            Params param = paramsService.findParamsByName(ParamsNames.MAX_ATTEMPTS_USER_CONNEXION.getCode());
            ...
            Params param = paramsService.findParamsByName(ParamsNames.TOKEN_AUTHENTIFICATION.getCode());
            param.setValue(selectedParam.getValue());
            paramsService.update(param);
        } else if (ParamsConstants.BACKUP_RECEVED_MESSAGES.getParamsName().equals(selectedParam.getName())) {
            Params param = null;
            if (!selectedParam.getValue().equals(Boolean.TRUE.toString()) && !selectedParam.getValue().equals(Boolean.FALSE.toString())) {
                param = paramsService.findParamsByName(ParamsConstants.BACKUP_RECEVED_MESSAGES.getParamsName());
                selectedParam.setValue(param.getValue());
                String msg = ResourceBundle.getBundle(LOCAL_BUNDLE_NAME, getCurrentLocale()).getString(BOOLEAN_ERROR);
            ...
            try {
                ...
                if (paramValue >= 0) {
                    ...
                    final String msg = ResourceBundle.getBundle(LOCAL_BUNDLE_NAME, getCurrentLocale()).getString(ParamsOrganism.class.getSimpleName() + PersistenceActions.UPDATE.getCode());
                ...
                final String msg = ResourceBundle.getBundle(LOCAL_BUNDLE_NAME, getCurrentLocale()).getString(PARAMS_ORGANISM_VALIDATION_ERROR);
                JsfUtil.addErrorMessage(msg);
            } catch (final Exception ex) {
                if (LOG.isDebugEnabled()) {
    ...
    public TreeNode populateParamsOrganismNode(final List<ParamsOrganism> list) {
        ...
        if (getLoggedUser().getAuthoritiesList().contains(AuthorityConstants.ROOT.getCode())) {
            final TreeNode authentificationAttemps = new DefaultTreeNode(new ParamsNode(CategoryNames.AUTHENTIFICATION_ATTEMPTS.getCode(), null, CATEGORY), parentRoot);
            authentificationAttemps.setExpanded(true);
            authentificationAttemps.setSelectable(false);
            final List<ParamsOrganism> generalsParamsOrganism = paramsOrganismService.findParamsOrganismByOrganism(getCurrentOrganism(), ParamsCategory.GN);
            ...
            final TreeNode cancelRequest = new DefaultTreeNode(new ParamsNode(CategoryNames.CANCEL_REQUEST.getCode(), null, CATEGORY), parentRoot);
        ...
        try {
            if (getLoggedUser().getAuthoritiesList().contains(AuthorityConstants.ROOT.getCode())) {
                if (paramsOrganism.getParam().getName().equalsIgnoreCase(ParamsNames.MAX_ATTEMPTS_USER_CONNEXION.getCode())) {
                    final TreeNode maxAttemptsUserConnexion = new DefaultTreeNode(convertParamsOrganism(paramsOrganism), treeNodeList.get(Constants.ZERO));
                    ...
                    final TreeNode tokenAuthentification = new DefaultTreeNode(convertParamsOrganism(paramsOrganism), treeNodeList.get(Constants.ZERO));
                    tokenAuthentification.setSelectable(false);
                } else if (paramsOrganism.getParam().getName().equals(ParamsConstants.BACKUP_RECEVED_MESSAGES.getParamsName())) {
                    final TreeNode backupRecMsgs = new DefaultTreeNode(convertParamsOrganism(paramsOrganism), treeNodeList.get(Constants.ZERO).getParent());
                    backupRecMsgs.setSelectable(false);
                }
            } else if (paramsOrganism.getParam().getName().equalsIgnoreCase(ParamsNames.MAX_CANCEL_REQUEST.getCode())) {
                final TreeNode maxCancelRequest = new DefaultTreeNode(convertParamsOrganism(paramsOrganism), treeNodeList.get(Constants.ZERO));
    ...
    public String bundle(ParamsNode paramsNode) {

        try {
            String msg = ResourceBundle.getBundle(ControllerConstants.Bundle.LOCAL_BUNDLE_NAME, getCurrentLocale()).getString(paramsNode.getName());
            return msg;
        } catch (Exception ex) {
        }

        if (paramsNode != null && paramsNode.getName().equals(ParamsConstants.BACKUP_RECEVED_MESSAGES.getParamsName())) {
            return getCurrentLocale().getLanguage().equals(Locale.ENGLISH.getLanguage())
                    ? ParamsConstants.BACKUP_RECEVED_MESSAGES.getParamsLabelEn()
                    : ParamsConstants.BACKUP_RECEVED_MESSAGES.getParamsLabelFr();
        }

        return paramsNode != null ? paramsNode.getName() : "";
    }
