...
public class ControllerHolidayController extends AbstractController<ControllerHoliday>
{
	...
	public void create()
	{
		if (selected.getFromDate() != null && selected.getToDate() != null && selected.getFromDate().after(selected.getToDate()))
		{
		...
		if (selected.getEnabled())
		{
			final List<ControllerHoliday> existsHolidays = controllerHolidayService.findHolidayByControllerAndDates(
					selected.getUser(), selected.getFromDate(), selected.getToDate());
	...
	public void edit()
	{
		...
		if (selected.getEnabled())
		{
			...
			if (existsHolidays.remove(selected) && CollectionUtils.isNotEmpty(existsHolidays))
			{
				JsfUtil.addErrorMessage(ResourceBundle.getBundle(LOCAL_BUNDLE_NAME, getCurrentLocale()).getString(
						CONTROLLER_ALREADY_ON_HOLIDAY));
				return;
	...
	public void resetFields()
	{
		...
		listUserParService = new ArrayList<User>();
	}
	...
	public void prepareEdit()
	{
		...
		if (selected.getUser().getAdministration() instanceof Bureau)
		{
			serviceSelected = ((Bureau) selected.getUser().getAdministration()).getService();
	 ...
	 *
	 * @param listUserParService
	 *           the listUserParService to set
	 */
	 ...
	 *
	 * @param bolSelectInspector
	 *           the bolSelectInspector to set
	 */
