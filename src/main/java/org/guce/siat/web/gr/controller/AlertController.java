...
public class AlertController extends AbstractController<Alert>
{
	...
	public void create()
	{
		if (selected.getServicesItem() != null || selected.getCountry() != null)
		{
			try
			{
				...
				selected.setSend(false);
				alertService.save(selected);
	...
	public void edit()
	{
		if (selected.getServicesItem() != null || selected.getCountry() != null)
		{
			try
			{
				...
				{
					selected.setCountryId(null);
				}
				selected.setSend(false);
				alertService.update(selected);
				if (!isValidationFailed())
				{
					initCreate();
					selectedAlert = null;
	 ...
	 * @return the document
	 */
	private DOCUMENT initHeader(final DOCUMENT alertDocument)
	{//init MESSAGE
	 ...
	 *
	 * @param flowGuceSiatService
	 *           the new flow guce siat service
	 */
